...
Ditto 是 DSH 的原生 Skill。使用者只要用自然語言說明目標，不必記住工具名稱；當請求涵蓋整個資料夾、多個模組、重複規則或「先看樣本再整批套用」時，Agent 會從 Skill 清單載入 Ditto 流程。單一檔案的普通修改與一般問答不會觸發。

例如，直接對 DSH 說：「把這個專案所有模組做成一致的 API 規格，先讓我審閱三份樣本。」Agent 會載入 Ditto，建立批次、準備樣本並等待你的核准。插件不保存模型金鑰，也不直接替你呼叫模型。
