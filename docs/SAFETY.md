...
| Symlink and junction escapes rejected in discovery and on the output side | `collectFiles`, `collectSourceFiles`, `prepareDestinationRoot`, `assertRealAncestor`, `safeOutput` | `tests/core/m0.test.ts` "rejects an output-root junction…"; discovery counts `symlink` exclusions |
