...
import { applyPlan, createPlan, defaultRecipe, loadPlan, recipeFromPlan, revisePlan, safeRelative, savePlan } from '../../src/core/index.js'
...
describe('M0 reviewed copies', () => {
  it('previews without writes, then copies exact bytes and preserves originals', async () => {
    ...
    expect(plan.summary.ready).toBe(2); await expect(readFile(join(paths.output, '文字', 'notes.txt'))).rejects.toMatchObject({ code: 'ENOENT' })
    ...
    expect(await readFile(join(paths.output, '文字', 'notes.txt'))).toEqual(before)
  ...
  it('rejects stale source bytes and never creates a stale output', async () => {
    ...
    const result = await applyPlan(plan, { id: plan.id, revision: plan.revision, digest: plan.digest }, paths.state)
    expect(result.items[0].status).toBe('rejected'); await expect(readFile(join(paths.output, '文字', 'notes.txt'))).rejects.toMatchObject({ code: 'ENOENT' })
  })
  ...
  it('rejects case-insensitive collisions and unsafe or extension-changing edits', async () => {
    ...
    expect(() => revisePlan(plan, [{ id: plan.items[0].id, destination: '文字/X.txt' }, { id: plan.items[1].id, destination: '文字/x.txt' }])).toThrow('same destination')
    expect(() => revisePlan(plan, [{ id: plan.items[0].id, destination: '文字/new.md' }])).toThrow('original .txt extension')
    for (const value of ['../escape.txt', 'CON.txt', 'report. ', 'folder/AUX.txt']) expect(() => safeRelative(value)).toThrow()
  })
  ...
  it('keeps extensionless names and dotfiles intact, including below dotted folders', async () => {
    ...
    const plan = await createPlan({ sourceRoot: paths.source, destinationRoot: paths.output, recipe: defaultRecipe() })
    expect(plan.items.map(item => item.destination).sort()).toEqual(['其他\\.keep', '其他\\README'])
    await savePlan(plan, paths.state)
    await applyPlan(plan, { id: plan.id, revision: plan.revision, digest: plan.digest }, paths.state)
    expect(await readFile(join(paths.output, '其他', 'README'), 'utf8')).toBe('read me')
    expect(await readFile(join(paths.output, '其他', '.keep'), 'utf8')).toBe('keep')
  })
  ...
  it('rejects an output-root junction before it can create files through it', async () => {
    ...
    await expect(readFile(join(outside, 'new-output', '文字', 'notes.txt'))).rejects.toMatchObject({ code: 'ENOENT' })
  })
  ...
  it('resumes recorded work and treats an unrecorded existing output as a non-overwrite failure', async () => {
    ...
    const first = plan.items[0]; await mkdir(join(paths.output, '文字'), { recursive: true }); await cp(first.source, join(paths.output, first.destination))
    ...
    await mkdir(join(separate.output, '文字'), { recursive: true }); await cp(unrecorded.items[0].source, join(separate.output, unrecorded.items[0].destination))
    ...
    expect(failed.items[0].status).toBe('failed'); expect(failed.items[0].reason).toContain('目的地已存在')
  })
  ...
  it('does not turn index template proposals into saved overrides', async () => {
    ...
    expect(reused.items[0].destination).toBe('文字\\copy-1.txt')
    const overridden = { ...saved, overrides: { 'three.txt': join('文字', 'custom.txt') } }
    const loaded = await createPlan({ sourceRoot: next.source, destinationRoot: join(next.root, 'other-output'), recipe: overridden })
    const reverted = revisePlan(loaded, [{ id: loaded.items[0].id, destination: '文字/copy-1.txt' }])
  ...
  it('serializes concurrent loopback mutations and rejects stale apply identities', async () => {
    ...
    try {
      ...
      const [apply, revise] = await Promise.all([
        ...
        fetch(`${server.url}/api/revise`, { method: 'POST', headers, body: JSON.stringify({ revision: plan.revision, edits: [{ id: plan.items[0].id, destination: '文字/renamed.txt' }] }) }),
      ])
