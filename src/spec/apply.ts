...
async function prepareOutputRoot(root: string): Promise<string> { const expected = resolve(root); await assertRealAncestor(expected); await mkdir(expected, { recursive: true }); const actual = await realpath(expected); const info = await lstat(expected); if (!info.isDirectory() || info.isSymbolicLink() || !samePath(actual, expected)) throw new Error('The specification output folder is a symlink, junction, or not a directory'); return actual }
async function assertRealAncestor(target: string): Promise<void> { let cursor = target; while (true) { try { const info = await lstat(cursor); const actual = await realpath(cursor); if (!info.isDirectory() || info.isSymbolicLink() || !samePath(actual, cursor)) throw new Error('The specification output folder has a symlink or junction ancestor'); return } catch (error: unknown) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; const parent = dirname(cursor); if (parent === cursor) throw new Error('The output folder has no accessible parent'); cursor = parent } } }
