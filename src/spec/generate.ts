import { readFile, lstat, mkdir, realpath, writeFile, rename } from 'node:fs/promises'
...
import type { GenerateSpecOptions, SpecBatch, SpecBatchEdit, SpecGenerationResponse, SpecGenerator, SpecModule } from './types.js'
...
export async function generateSpecBatch(batch: SpecBatch, generator: SpecGenerator, options: GenerateSpecOptions = {}): Promise<SpecBatch> {
  assertSpecBatch(batch); if (!generator || typeof generator.id !== 'string' || generator.id.length < 1 || generator.id.length > 160 || typeof generator.generate !== 'function') throw new Error('generator contract 不正確')
  const only = options.only ?? 'samples'
  if (only === 'remaining' && !hasApprovedCurrentSamples(batch)) throw new Error('請先核准目前的三個校準樣本')
...
export function submitSpecDraft(batch: SpecBatch, input: { moduleId: string; draft: unknown; metadata?: Record<string, string>; generatorId?: string }): SpecBatch {
  ...
  if (!item || !['pending', 'needs-review'].includes(item.status)) throw new Error('這個模組目前不能提交草稿')
  validateMetadata(input.metadata); validateSpecDraft(input.draft, item)
  const draft = input.draft as import('./types.js').SpecDraft; const markdown = renderSpecDraft(draft, item)
...
export function reviseSpecBatch(batch: SpecBatch, edit: SpecBatchEdit): SpecBatch {
  assertSpecBatch(batch); if (!edit || typeof edit !== 'object') throw new Error('批次修改不正確')
  ...
  if (changes.size !== (edit.sampleEdits?.length ?? 0) || [...changes.keys()].some(id => !batch.samples.includes(id))) throw new Error('只能修改三個校準樣本')
  ...
  for (const [id, markdown] of changes) {
    ...
    if (!['sample-ready', 'approved'].includes(item.status)) throw new Error('樣本尚未產生規格')
  ...
  if (batch.items.some(item => item.status === 'applied')) throw new Error('已有寫入結果，不能重新校準樣本或說明')
...
export function approveSpecSamples(batch: SpecBatch): SpecBatch {
  ...
  if (batch.items.some(item => item.status === 'applied')) throw new Error('已有寫入結果，不能重新核准樣本')
  if (!batch.samples.every(id => batch.items.find(item => item.id === id)?.status === 'sample-ready')) throw new Error('三個樣本都必須先產生規格並完成檢視')
  const approvedSamples = batch.samples.map(id => {
    ...
    if (!markdown) throw new Error('樣本尚未產生規格')
...
function hasApprovedCurrentSamples(batch: SpecBatch): boolean {
  return hasCurrentApprovalRecord(batch) && batch.samples.every(id => batch.items.find(candidate => candidate.id === id)?.status === 'approved')
}
function hasCurrentApprovalRecord(batch: SpecBatch): boolean {
...
export function assertSpecBatch(batch: unknown): asserts batch is SpecBatch {
  if (!batch || typeof batch !== 'object') throw new Error('規格批次不正確')
  const value = batch as SpecBatch
  if (value.version !== 'm1' || typeof value.id !== 'string' || !/^[0-9a-f-]{16,64}$/i.test(value.id) || !Number.isInteger(value.revision) || value.revision < 1 || !/^[a-f0-9]{64}$/i.test(value.digest) || !Array.isArray(value.items) || value.items.length < 1 || value.items.length > 50 || !Array.isArray(value.samples) || value.samples.length !== 3 || new Set(value.samples).size !== 3 || value.samples.some(id => !value.items.some(item => item.id === id)) || !value.recipe || value.recipe.version !== 1 || !value.discovery || value.discovery.inScope !== value.items.length || !Number.isInteger(value.discovery.excludedTotal) || !Array.isArray(value.discovery.excluded)) throw new Error('規格批次不正確')
  ...
  if (approvedIds.length > 3 || new Set(approvedIds).size !== approvedIds.length || approvedIds.some(id => !value.samples.includes(id)) || (approvedIds.length === 0 && value.recipe.approvalRevision !== undefined) || (approvedIds.length > 0 && value.recipe.approvalRevision !== value.revision)) throw new Error('規格批次的核准樣本不正確')
  ...
  const hasApprovalRecord = hasCurrentApprovalRecord(value)
  if (value.items.some(item => item.status === 'approved' && (!value.samples.includes(item.id) || !item.approvedMarkdown)) || value.items.some(item => !value.samples.includes(item.id) && ['ready', 'applied'].includes(item.status) && (!hasApprovalRecord || item.generation?.recipeDigest !== recipeDigest(value)))) throw new Error('規格批次跳過了樣本核准流程')
  if (withSpecDigest({ ...value, digest: '' }).digest !== value.digest) throw new Error('規格批次的檢視摘要不正確')
}

function assertSpecModule(item: SpecModule): void {
  if (!item || typeof item.id !== 'string' || typeof item.source !== 'string' || typeof item.relativePath !== 'string' || typeof item.outputPath !== 'string' || !/^[a-f0-9]{64}$/i.test(item.sourceHash) || !['pending', 'sample-ready', 'approved', 'ready', 'applied', 'needs-review', 'rejected', 'failed'].includes(item.status) || !Array.isArray(item.evidence) || !item.evidence.length) throw new Error('規格模組不正確')
  if (item.draft) validateSpecDraft(item.draft, item)
  if (item.renderedMarkdown && (!item.renderedHash || renderedHash(item.renderedMarkdown) !== item.renderedHash)) throw new Error('規格 Markdown 摘要不正確')
}
function validateMetadata(metadata: unknown): void { if (metadata === undefined) return; if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata) || Object.keys(metadata).length > 30 || Object.entries(metadata as Record<string, unknown>).some(([key, value]) => key.length > 80 || typeof value !== 'string' || value.length > 500)) throw new Error('產生中繼資料不正確') }
...
async function generateOne(batch: SpecBatch, item: SpecModule, generator: SpecGenerator, stateRoot?: string): Promise<SpecModule> {
  try {
    ...
    const response = cached ?? await generator.generate({ module: { id: item.id, relativePath: item.relativePath, sourceHash: item.sourceHash, evidence: item.evidence, facts: item.facts }, instructions: batch.recipe.instructions, approvedSamples: batch.recipe.approvedSamples })
  ...
  } catch (error: unknown) { return { ...item, status: 'needs-review', reason: (error instanceof Error ? error.message : '規格產生失敗').slice(0, 300) } }
}
